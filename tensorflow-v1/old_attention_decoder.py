...
def attention_decoder(
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
        ...
        for i, inp in enumerate(decoder_inputs):
            logging.info("Adding attention_decoder timestep %i of %i", i, len(decoder_inputs))
            if i > 0:
            ...
            cell_output, (state_h, state_c) = cell(x, state)
...
def linear(args, output_size: int, bias: bool, bias_start=0.0, scope=None):
    ...
    with tf.compat.v1.variable_scope(scope or "Linear"):
        ...
        # res = keras.ops.matmul(args, matrix)
        if not bias:
            return res
        bias_term = tf.compat.v1.get_variable(
            "Bias", [output_size], initializer=tf.compat.v1.constant_initializer(int(bias_start))
        )
